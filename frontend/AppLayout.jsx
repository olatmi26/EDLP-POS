...
const FLYOUT_MENUS = {
  ...
  customers: {
    ...
    sections: [
      ...
      { label: 'Import / Export', items: [
        { icon: '📥', label: 'Import Customers',     sub: 'CSV bulk import',          to: '/customers', isNew: true },
        { icon: '📤', label: 'Export Customers',     sub: 'Download CSV',             to: '/customers' },
      ]},
    ],
  },
  wholesale: {
...
export function AppLayout() {
  ...
  const switchBranchMut = useMutation({
    ...
    onSuccess: async res => {
      ...
      toast.success(branchName ? `Switched to ${branchName}` : 'Viewing all branches')
      setUserMenu(false)
    },
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--edlp-bg)' }}>
      ...
      <aside className="edlp-sidebar">
        ...
        {user?.branch ? (
          ...
          <div style={{ margin: '10px 12px 0', padding: '6px 10px', background: 'rgba(26,63,166,0.08)', border: '1px solid rgba(26,63,166,0.2)', borderRadius: 8, color: '#1A3FA6', fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 0 }}>
            🌐 All Branches
          </div>
        ) : null}
                  ...
                  <div style={{ marginBottom: 10, paddingBottom: 10, borderBottom: '1px solid #F1F5F9' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <select
                        ...
                        onChange={e => {
                          // empty string → null (all branches), else numeric id
