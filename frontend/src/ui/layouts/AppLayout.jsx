...
import {
  ...
  Layers, BookOpen, GitBranch, TrendingUp, MoveHorizontal,
  Tag,
} from 'lucide-react'
...
const NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/stock-movements',icon: MoveHorizontal, label: 'Stock Movements' },
    ],
  },
  {
    label: 'Purchasing',
...
function SideNavItem({ to, icon: Icon, label, collapsed, badge }) {
  return (
    <div style={{ position: 'relative' }}>
      ...
      {badge > 0 && !collapsed && (
        <span style={{ position: 'absolute', top: 6, right: 8, background: '#C0392B', color: '#fff', fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 10, pointerEvents: 'none', minWidth: 16, textAlign: 'center' }}>
          {badge > 99 ? '99+' : badge}
        </span>
      )}
    </div>
...
export function AppLayout() {
  ...
  // Fetch all branches for switcher (admin+ only)
  ...
  // Approval badge count — refreshes every 60s
  ...
  const currentBranch = user?.branch?.name ?? '—'
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <header style={{ height: 56, background: '#fff', borderBottom: '1px solid #E5EBF2', display: 'flex', alignItems: 'center', gap: 12, padding: '0 20px', flexShrink: 0 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            {userMenuOpen && (
              <div
                style={{ position: 'absolute', right: 0, top: 44, background: '#fff', border: '1px solid #E5EBF2', borderRadius: 10, boxShadow: '0 10px 30px rgba(15,23,42,0.12)', minWidth: 230, zIndex: 100, padding: 12 }}
              ...
              >
                ...
                {isAdminLike && (
                  <div style={{ marginBottom: 10, paddingBottom: 10, borderBottom: '1px solid #F1F5F9' }}>
                    <div style={{ fontSize: 11, color: '#8A9AB5', marginBottom: 6, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Switch Branch</div>
