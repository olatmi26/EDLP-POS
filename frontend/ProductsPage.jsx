/**
 * ProductsPage v4 — EDLP POS Product Catalog
 *
 * Fixed vs v3:
 *  ✅ JSX parse error fixed — DataTable + pagination wrapped in <> fragment
 *  ✅ Template literal backtick in standalone image upload onChange fixed
...
function ShimmerStyle() {
  return (
    <style>
      {`@keyframes edlp-shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}`}
    </style>
  )
}
...
function ProductThumb({ url, name }) {
  return (
    <div style={{
      ...
      border: '1px solid #E5EBF2', overflow: 'hidden',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
    }}>
      {url
        ? <img src={url} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        : <ImageIcon size={13} color="#D5DFE9" />
      }
    </div>
  )
}
...
function Tabs({ tabs, active, onChange }) {
  return (
    <div style={{
      display: 'flex', gap: 4, background: '#F0F4F8',
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        ...
        <span style={{
          fontSize: 11, fontWeight: 700, color: '#8A9AB5',
    ...
    <div style={{ marginTop: 4 }}>
      {!open ? (
        <button
          ...
          style={{
            fontSize: 11, color: '#1A3FA6', background: 'none',
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 4 }}>
          <input
            ...
            style={{
              ...
              border: '1px solid #D5DFE9', borderRadius: 6, outline: 'none',
            }}
            autoFocus
          />
          <button
            ...
            style={{
              ...
              background: '#0A1628', color: '#E8A020',
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer', color: '#8A9AB5', borderRadius: 4,
            }}
          >
            <X size={12} />
          </button>
...
// ─────────────────────────────────────────────────────────────────────────────
// Analytics Tab — full recharts-powered dashboard
// ─────────────────────────────────────────────────────────────────────────────

const CHART_COLORS = ['#E8A020', '#1A3FA6', '#1A6E3A', '#C0392B', '#7B2FBE', '#0891B2', '#C45A00', '#059669']

function AnalyticsTab({ products, loading, branches, branchId }) {
  // ── Derived data ────────────────────────────────────────────────────────────
  const categoryData = useMemo(() => {
    const map = {}
    products.forEach(p => {
      const cat = p.category?.name ?? 'Uncategorised'
      if (!map[cat]) map[cat] = { name: cat, total: 0, low: 0, out: 0, ok: 0, value: 0 }
      map[cat].total++
      map[cat].value += (p.stock ?? 0) * p.selling_price
      if (p.stock_status === 'low')  map[cat].low++
      else if (p.stock_status === 'out') map[cat].out++
      else if (p.stock_status === 'ok')  map[cat].ok++
    })
    return Object.values(map).sort((a, b) => b.total - a.total).slice(0, 10)
  }, [products])

  const topMarginData = useMemo(() =>
    products
      .filter(p => p.cost_price > 0 && p.selling_price > 0)
      .map(p => ({
  ...
  const stockHealthData = useMemo(() => {
    ...
    const low      = products.filter(p => p.stock_status === 'low').length
    const out      = products.filter(p => p.stock_status === 'out').length
    const unknown  = products.filter(p => !p.stock_status || p.stock_status === 'unknown').length
    return [
      { name: 'In Stock',    value: ok,      fill: '#1A6E3A' },
      { name: 'Low Stock',   value: low,     fill: '#C45A00' },
      { name: 'Out',         value: out,     fill: '#C0392B' },
      { name: 'Not Tracked', value: unknown, fill: '#B0BCC8' },
    ].filter(d => d.value > 0)
  }, [products])

  const branchStockData = useMemo(() => {
    if (!products.length) return []
    // aggregate total stock per branch across all products
    const branchMap = {}
    products.forEach(p => {
      (p.branch_stocks ?? []).forEach(bs => {
        if (!branchMap[bs.branch_id]) branchMap[bs.branch_id] = { branchId: bs.branch_id, total: 0, products: 0 }
        branchMap[bs.branch_id].total += bs.quantity
        branchMap[bs.branch_id].products++
      })
    })
    return Object.values(branchMap)
      .map(b => ({
        ...b,
        name: branches.find(br => br.id === b.branchId)?.name ?? `Branch ${b.branchId}`,
      }))
  ...
  const priceRangeData = useMemo(() => {
    ...
    return ranges.map(r => ({
      name:  r.label,
      count: products.filter(p => p.selling_price >= r.min && p.selling_price < r.max).length,
    })).filter(r => r.count > 0)
  }, [products])

  const summaryStats = useMemo(() => {
    const tracked = products.filter(p => p.stock !== null && p.stock !== undefined)
    const totalStockValue = tracked.reduce((s, p) => s + (p.stock ?? 0) * p.cost_price, 0)
    const avgMargin = products.filter(p => p.cost_price > 0 && p.selling_price > 0)
      .reduce((s, p, _, arr) => s + ((p.selling_price - p.cost_price) / p.selling_price * 100) / arr.length, 0)
    return {
      totalSKUs:        products.length,
      trackedSKUs:      tracked.length,
      totalStockValue,
      avgMargin:        Math.round(avgMargin),
      categories:       [...new Set(products.map(p => p.category?.name).filter(Boolean))].length,
      suppliers:        [...new Set(products.map(p => p.supplier?.name).filter(Boolean))].length,
    }
  }, [products])

  if (loading) return (
    <div className="flex items-center justify-center py-20">
      <Spinner size={28} color="#E8A020" />
    </div>
  )
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div style={{ background: '#1C2B3A', borderRadius: 8, padding: '8px 12px', fontSize: 12, color: '#fff', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}>
        <div style={{ color: 'rgba(255,255,255,0.55)', marginBottom: 4 }}>{label}</div>
        {payload.map((p, i) => (
          <div key={i} style={{ color: p.color ?? '#E8A020', fontWeight: 700 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12 }}>
        {[
          { label: 'Total SKUs',       value: summaryStats.totalSKUs,                     color: '#1A3FA6' },
          { label: 'Tracked SKUs',     value: summaryStats.trackedSKUs,                   color: '#1A6E3A' },
          { label: 'Stock Value',      value: `₦${(summaryStats.totalStockValue/1000).toFixed(0)}k`, color: '#E8A020' },
          { label: 'Avg Margin',       value: `${summaryStats.avgMargin}%`,                color: summaryStats.avgMargin >= 20 ? '#1A6E3A' : summaryStats.avgMargin >= 10 ? '#C45A00' : '#C0392B' },
          { label: 'Categories',       value: summaryStats.categories,                    color: '#7B2FBE' },
          { label: 'Suppliers',        value: summaryStats.suppliers,                     color: '#0891B2' },
        ].map(({ label, value, color }) => (
          <div key={label} style={{ background: '#fff', border: '1px solid #E5EBF2', borderRadius: 12, padding: '14px 18px' }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#8A9AB5', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>{label}</div>
            <div style={{ fontSize: 22, fontWeight: 800, color }}>{value}</div>
          </div>
        ))}
      </div>

      {/* ── Row 1: Category bar + Margin horizontal bar ──────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>

        <Card style={{ padding: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#1C2B3A', marginBottom: 4 }}>Products by Category</div>
          <div style={{ fontSize: 12, color: '#8A9AB5', marginBottom: 16 }}>Top 10 categories — stock health breakdown</div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={categoryData} layout="vertical" margin={{ left: 10, right: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F0F4F8" horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11, fill: '#8A9AB5' }} />
              <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fill: '#3A4A5C' }} width={90} />
              <RechartTooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={{ fontSize: 11 }} />
              <Bar dataKey="ok"    name="In Stock"  stackId="a" fill="#1A6E3A" radius={[0,0,0,0]} />
              <Bar dataKey="low"   name="Low Stock" stackId="a" fill="#C45A00" />
              <Bar dataKey="out"   name="Out"       stackId="a" fill="#C0392B" radius={[0,3,3,0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>

        <Card style={{ padding: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#1C2B3A', marginBottom: 4 }}>Top 10 Products by Margin</div>
          <div style={{ fontSize: 12, color: '#8A9AB5', marginBottom: 16 }}>Gross margin % — higher is better</div>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={topMarginData} layout="vertical" margin={{ left: 10, right: 30 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F0F4F8" horizontal={false} />
              <XAxis type="number" unit="%" tick={{ fontSize: 11, fill: '#8A9AB5' }} domain={[0, 'auto']} />
              <YAxis type="category" dataKey="name" tick={{ fontSize: 10, fill: '#3A4A5C' }} width={110} />
              <RechartTooltip content={<CustomTooltip />} />
              <Bar dataKey="margin" name="Margin %" radius={[0, 4, 4, 0]}>
                {topMarginData.map((entry, i) => (
                  <Cell key={i} fill={entry.margin >= 30 ? '#1A6E3A' : entry.margin >= 15 ? '#E8A020' : '#C0392B'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      {/* ── Row 2: Stock health donut + Branch stock + Price range ────── */}
      <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr 1fr', gap: 20 }}>

        <Card style={{ padding: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#1C2B3A', marginBottom: 4 }}>Stock Health</div>
          <div style={{ fontSize: 12, color: '#8A9AB5', marginBottom: 12 }}>Catalogue-wide status</div>
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
              <Pie data={stockHealthData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3}>
                {stockHealthData.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
              </Pie>
              <RechartTooltip formatter={(v, n) => [`${v} SKUs`, n]} />
        ...
        <Card style={{ padding: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#1C2B3A', marginBottom: 4 }}>Stock by Branch</div>
          <div style={{ fontSize: 12, color: '#8A9AB5', marginBottom: 16 }}>Total units held per branch</div>
          {branchStockData.length === 0 ? (
            <div style={{ padding: '40px 0', textAlign: 'center', color: '#B0BCC8', fontSize: 13 }}>No multi-branch stock data</div>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={branchStockData} margin={{ left: 0, right: 10, top: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#F0F4F8" />
                <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#8A9AB5' }} />
                <YAxis tick={{ fontSize: 11, fill: '#8A9AB5' }} />
                <RechartTooltip content={<CustomTooltip />} />
                <Bar dataKey="total" name="Units" radius={[4,4,0,0]}>
                  {branchStockData.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          )}
        ...
        <Card style={{ padding: 20 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#1C2B3A', marginBottom: 4 }}>Price Range Distribution</div>
          <div style={{ fontSize: 12, color: '#8A9AB5', marginBottom: 16 }}>Number of SKUs in each price band</div>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={priceRangeData} margin={{ left: 0, right: 10, top: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#F0F4F8" />
              <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#8A9AB5' }} />
              <YAxis tick={{ fontSize: 11, fill: '#8A9AB5' }} />
              <RechartTooltip content={<CustomTooltip />} />
              <Bar dataKey="count" name="SKUs" fill="#E8A020" radius={[4,4,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        </Card>
      </div>

      {/* ── Row 3: Value by category table ───────────────────────────── */}
      <Card style={{ padding: 0 }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #F0F4F8', fontSize: 14, fontWeight: 700, color: '#1C2B3A' }}>
          Category Value Analysis
          <span style={{ fontSize: 12, fontWeight: 400, color: '#8A9AB5', marginLeft: 8 }}>Stock × cost price per category</span>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ background: '#F8FAFC' }}>
                {['Category', 'SKUs', 'In Stock', 'Low', 'Out', 'Avg Margin', 'Stock Value'].map(h => (
                  <th key={h} style={{ padding: '10px 16px', textAlign: h === 'Category' ? 'left' : 'right', fontSize: 11, fontWeight: 700, color: '#8A9AB5', textTransform: 'uppercase', borderBottom: '1px solid #F0F4F8' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {categoryData.map((row, i) => {
                const catProds = products.filter(p => (p.category?.name ?? 'Uncategorised') === row.name)
                const avgM = catProds.filter(p => p.cost_price > 0 && p.selling_price > 0)
                  .reduce((s, p, _, arr) => arr.length ? s + ((p.selling_price - p.cost_price) / p.selling_price * 100) / arr.length : 0, 0)
                return (
                  <tr key={row.name} style={{ borderBottom: '0.5px solid #F8FAFC' }}>
                    <td style={{ padding: '10px 16px', fontWeight: 600, color: '#1C2B3A' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 8, height: 8, borderRadius: '50%', background: CHART_COLORS[i % CHART_COLORS.length], flexShrink: 0 }} />
                        {row.name}
                      </div>
                    </td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', color: '#3A4A5C' }}>{row.total}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', color: '#1A6E3A', fontWeight: 600 }}>{row.ok}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', color: '#C45A00', fontWeight: 600 }}>{row.low}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', color: '#C0392B', fontWeight: 600 }}>{row.out}</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', color: Math.round(avgM) >= 20 ? '#1A6E3A' : Math.round(avgM) >= 10 ? '#C45A00' : '#C0392B', fontWeight: 700 }}>{Math.round(avgM)}%</td>
                    <td style={{ padding: '10px 16px', textAlign: 'right', fontWeight: 700, color: '#1C2B3A' }}>₦{(row.value / 1000).toFixed(0)}k</td>
                  </tr>
                )
              })}
            </tbody>
...
export function ProductsPage() {
  ...
  const allProductsQ = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get('/products', {
        params: { all: true, per_page: 500, active_only: false },
      })
  ...
  function openCreate() {
    ...
    reset({
      ...
      unit: 'pcs', reorder_level: 10,
      is_active: true, is_vat_exempt: false,
    })
  ...
  function openEdit(p) {
    ...
    reset({
      ...
      unit: p.unit ?? 'pcs', reorder_level: p.reorder_level ?? 10,
  ...
  const saveMut = useMutation({
    mutationFn: async (data) => {
      ...
      if (pendingImage) {
        ...
        await api.post(
          `/products/${res.data?.data?.id ?? editProduct.id}/image`,
          fd,
  ...
  async function doExport() {
    try {
      ...
      a.href = url
      a.download = `edlp-products-${new Date().toISOString().slice(0, 10)}.csv`
      a.click()
  ...
  const openingStockMut = useMutation({
    mutationFn: ({ file, branchId }) => {
      const fd = new FormData()
      fd.append('file', file)
      fd.append('branch_id', branchId)
      return api.post('/products/opening-stock', fd, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
    },
    onSuccess: res => {
      ...
      toast.success(`Opening stock applied: ${d?.updated ?? 0} products updated`)
      setOpeningStockModal(false)
      setOsFile(null)
      setOsBranch('')
    },
    onError: e => toast.error(e?.response?.data?.message ?? 'Opening stock import failed'),
  })

  function downloadOpeningStockTemplate() {
    const headers = ['sku*', 'quantity*', 'cost_price', 'notes']
    const example = ['IND-CHK-70G', '100', '320', 'Initial stock count']
    const csv = [headers, example].map(r => r.join(',')).join('\n')
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv' }))
    const a   = document.createElement('a')
    a.href = url
    a.download = 'edlp-opening-stock-template.csv'
    a.click()
    URL.revokeObjectURL(url)
  ...
  function downloadTemplate() {
    ...
    const a   = document.createElement('a')
    a.href = url
    a.download = 'edlp-products-import-template.csv'
    a.click()
    URL.revokeObjectURL(url)
    toast.success('Template downloaded')
  }
      ...
      cell: p => (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{
              fontWeight: 600, color: '#1C2B3A', fontSize: 13,
    ...
    },
    {
      ...
      cell: p => (
        <div>
          ...
          {p.barcode && (
            <div style={{ fontFamily: 'monospace', fontSize: 11, color: '#8A9AB5' }}>{p.barcode}</div>
          )}
        </div>
      ),
    },
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <span style={{
              ...
              borderRadius: 20, background: cfg.bg, color: cfg.color, width: 'fit-content',
            }}>{cfg.label}</span>
            {!p.is_active && (
              <span style={{ fontSize: 10, color: '#8A9AB5', fontWeight: 600 }}>Inactive</span>
            )}
          </div>
        )
      },
        ...
        if (stocks.length > 1) {
          return (
            <div style={{ fontSize: 11 }}>
              ...
              {stocks.length > 2 && (
                <div style={{ color: '#8A9AB5' }}>+{stocks.length - 2} more</div>
              )}
            </div>
          )
        }
    ...
    ...(isAdminLike ? [{
      ...
      cell: p => (
        <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
          <button
            title="Upload image"
            onClick={e => { e.stopPropagation(); setImageTarget(p) }}
            style={{
              padding: 5, borderRadius: 6, border: '1px solid #E5EBF2',
              background: 'transparent', cursor: 'pointer', color: '#8A9AB5',
          ...
          <button
            ...
            style={{
              padding: 5, borderRadius: 6, border: '1px solid #E5EBF2',
              background: 'transparent', cursor: 'pointer', color: '#1A3FA6',
              display: 'flex', alignItems: 'center',
            }}
          >
            <Pencil size={13} />
          ...
          <button
            ...
            style={{
              padding: 5, borderRadius: 6, border: '1px solid #FDECEA',
              background: 'transparent', cursor: 'pointer', color: '#C0392B',
              display: 'flex', alignItems: 'center',
            }}
          >
            <Trash2 size={13} />
  ...
  const bulkColumns = [
    {
      ...
      header: (
        <input
          type="checkbox"
          checked={selectedForBulk.length === products.length && products.length > 0}
  ...
  function MarginPreview() {
    ...
    const profit = sp - cp
    const color  = margin >= 20 ? '#1A6E3A' : margin >= 10 ? '#C45A00' : '#C0392B'
    const bg     = margin >= 20 ? '#EAF5EE' : margin >= 10 ? '#FEF0E6' : '#FDECEA'
    return (
      <div style={{ display: 'flex', gap: 16, padding: '10px 14px', background: bg, borderRadius: 8, fontSize: 12 }}>
        <span style={{ color: '#8A9AB5' }}>Gross Margin:</span>
        <span style={{ fontWeight: 700, color }}>{margin}%</span>
        <span style={{ color: '#8A9AB5' }}>Profit per unit:</span>
        <span style={{ fontWeight: 700, color }}>{money(profit)}</span>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: '#8A9AB5' }}>
            ...
            Manage products, pricing, stock levels and analytics.
          </p>
        </div>
        {isAdminLike && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <Btn variant="ghost" onClick={() => setExportModal(true)}>
              <Download size={14} /> Export CSV
            </Btn>
            <Btn variant="ghost" onClick={() => {
              setImportStep(1); setImportFile(null); setImportPreview(null); setImportModal(true)
            }}>
              <Upload size={14} /> Import CSV
            </Btn>
            <Btn variant="ghost" onClick={() => { setOsFile(null); setOsBranch(''); setOpeningStockModal(true) }}>
              <Package size={14} /> Opening Stock
            </Btn>
            <Btn variant="ghost" onClick={() => setPrintLabelModal(true)}>
              <Tag size={14} /> Print Labels
            </Btn>
            <Btn variant="ghost" onClick={() => setBulkModal(true)}>
              <FileSpreadsheet size={14} /> Bulk Price
            </Btn>
      ...
      {/* ── Tabs ────────────────────────────────────────────────────────── */}
      <Tabs
        tabs={['Product List', 'Price History', 'Analytics']}
        active={activeTab}
      ...
      {activeTab === 'Product List' && (
        <Card style={{ padding: 0 }}>
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', flex: '1 1 220px', maxWidth: 340 }}>
              <Search size={13} style={{
                ...
                transform: 'translateY(-50%)', color: '#8A9AB5', pointerEvents: 'none',
              }} />
              <input
                value={search}
                ...
                style={{
                  ...
                  border: '1px solid #E5EBF2', borderRadius: 8, outline: 'none',
                  boxSizing: 'border-box', color: '#3A4A5C',
                }}
            ...
            <select
              ...
              style={{
                padding: '8px 10px', fontSize: 13, border: '1px solid #E5EBF2',
                borderRadius: 8, color: '#3A4A5C', background: '#fff', outline: 'none',
              }}
            >
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  background: statusFilter === v ? '#0A1628' : '#F0F4F8',
                  color: statusFilter === v ? '#fff' : '#6B7A8D',
                  transition: 'all 0.15s',
                }}
              >
                {label}
            ...
            <label style={{
              ...
              cursor: 'pointer', fontSize: 12, color: '#6B7A8D',
              ...
              <input
                ...
                style={{ accentColor: '#E8A020', width: 13, height: 13 }}
              />
              Active only
            </label>
            ...
            <button
              ...
              style={{
                padding: 7, borderRadius: 7, border: '1px solid #E5EBF2',
                background: 'transparent', cursor: 'pointer', color: '#8A9AB5',
                display: 'flex', alignItems: 'center',
              }}
            >
          ...
          {/* Table — fragment fixes the v3 parse error */}
          {productsQ.isLoading ? (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #F0F4F8' }}>
                  {['Product', 'SKU / Barcode', 'Price', 'Stock Level', 'Status', ''].map(h => (
                    <th key={h} style={{
                      padding: '10px 16px', textAlign: 'left',
                      fontSize: 11, fontWeight: 700, color: '#8A9AB5', textTransform: 'uppercase',
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
            ...
            <>
              ...
              {meta && meta.last_page > 1 && (
                <div style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  gap: 4, padding: '14px 16px', borderTop: '1px solid #F0F4F8',
                }}>
                  <button
                    onClick={() => setPage(p => Math.max(1, p - 1))}
                    disabled={meta.current_page <= 1}
                    style={{
                      padding: '5px 10px', borderRadius: 6, border: '1px solid #E5EBF2',
                      background: '#fff',
                      cursor: meta.current_page <= 1 ? 'not-allowed' : 'pointer',
                      color: meta.current_page <= 1 ? '#D5DFE9' : '#3A4A5C',
                  ...
                  <button
                    ...
                    style={{
                      ...
                      color: meta.current_page >= meta.last_page ? '#D5DFE9' : '#3A4A5C',
                      fontSize: 12, fontWeight: 600,
                    }}
                  >→</button>

                  <span style={{ fontSize: 11, color: '#8A9AB5', marginLeft: 8 }}>
                    {meta.total} products
                  </span>
                </div>
              )}
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ padding: '11px 14px' }}>
                            <code style={{ fontSize: 12, color: '#6B7A8D' }}>{p.sku}</code>
                          </td>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          ...
          ══════════════════════════════════════════════════════════════════ */}
      {activeTab === 'Analytics' && (
        <AnalyticsTab products={allProducts} loading={allProductsQ.isLoading} branches={branches} branchId={branchId} />
      )}
      ...
      <Modal
        ...
        footer={
          <>
            <Btn variant="ghost" onClick={() => setProductModal(false)} disabled={saveMut.isPending}>
              Cancel
            </Btn>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{
            ...
            borderRadius: 10, border: '1px dashed #D5DFE9',
          }}>
            <div style={{
            ...
            <label style={{
              padding: '7px 16px', background: '#0A1628', color: '#E8A020',
          ...
          <div style={{ fontSize: 11, fontWeight: 700, color: '#8A9AB5', textTransform: 'uppercase', letterSpacing: '0.07em' }}>
            Basic Information
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <FormField label="Unit of Measure">
              <FormSelect register={register('unit')}>
                ...
                {units.map(u => (
                  <option key={u.id} value={u.short_code}>{u.name} ({u.short_code})</option>
                ))}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <div>
              ...
              <QuickAddInline
                ...
                onAdd={name => addCategoryMut.mutate(name)}
                loading={addCategoryMut.isPending}
              />
            </div>
            <div>
              ...
              <QuickAddInline
                ...
                onAdd={name => addSupplierMut.mutate(name)}
                loading={addSupplierMut.isPending}
              />
            </div>
          ...
          <FormField label="Description">
            <textarea
              ...
              style={{
                ...
                border: '1px solid #D5DFE9', borderRadius: 8, outline: 'none',
                boxSizing: 'border-box', color: '#3A4A5C', resize: 'vertical',
                fontFamily: 'inherit',
              }}
              onFocus={e => e.target.style.borderColor = '#E8A020'}
          ...
          <div style={{ display: 'flex', gap: 20 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 7, cursor: 'pointer', fontSize: 13, color: '#3A4A5C' }}>
              <input type="checkbox" {...register('is_active')} style={{ width: 14, height: 14, accentColor: '#E8A020' }} />
              Active product
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: 7, cursor: 'pointer', fontSize: 13, color: '#3A4A5C' }}>
              <input type="checkbox" {...register('is_vat_exempt')} style={{ width: 14, height: 14, accentColor: '#E8A020' }} />
              VAT Exempt
            </label>
          </div>
        </div>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          BULK PRICE UPDATE MODAL — never closes on backdrop click
          ══════════════════════════════════════════════════════════════════ */}
      {bulkModal && (
        <div
          style={{
        ...
        >
          <div
            style={{
              background: '#fff', borderRadius: 14, width: '90%', maxWidth: 700,
          ...
          >
            ...
            <div style={{
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8A9AB5', padding: 4, borderRadius: 6 }}
              >
                <X size={18} />
              </button>
            </div>
            ...
            {/* Mode tabs */}
            <div style={{ display: 'flex', borderBottom: '1px solid #F0F4F8', flexShrink: 0 }}>
              {[
                { id: 'multi',  label: '🔢 Select Products', desc: 'Pick products manually' },
                { id: 'upload', label: '📁 Upload File',     desc: 'CSV / Excel bulk update' },
              ].map(m => (
                <button
                  key={m.id}
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    borderBottom: bulkMode === m.id ? '2px solid #E8A020' : '2px solid transparent',
                    color: bulkMode === m.id ? '#1C2B3A' : '#8A9AB5',
                    fontWeight: 600, fontSize: 13, textAlign: 'left', marginBottom: -1,
                  }}
                >
                  {m.label}
                  <div style={{ fontSize: 10, fontWeight: 400, color: '#B0BCC8', marginTop: 1 }}>{m.desc}</div>
                </button>
              ))}
            </div>
              ...
              {bulkMode === 'multi' ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    <div>
                      ...
                      <select
                        ...
                        style={{
                          ...
                          border: '1px solid #D5DFE9', borderRadius: 8, outline: 'none', background: '#fff',
                        }}
                      >
                    ...
                    <div>
                      <label style={{ fontSize: 12, fontWeight: 600, color: '#3A4A5C', display: 'block', marginBottom: 6 }}>
                        Value * {bulkType === 'percentage' ? '(e.g. 10 = +10%)' : bulkType === 'markup' ? '(e.g. 40 = 40% markup)' : '(New price ₦)'}
                      </label>
                      <input
                        type="number" step="0.01"
                        value={bulkValue}
                        onChange={e => setBulkValue(e.target.value)}
                        placeholder={bulkType === 'percentage' ? '10' : '750'}
                        style={{
                          width: '100%', padding: '8px 10px', fontSize: 13,
                          border: '1px solid #D5DFE9', borderRadius: 8, outline: 'none', boxSizing: 'border-box',
                        }}
                      />
                    </div>
                    <div>
                      ...
                      <select
                        value={bulkCatFilter}
                        onChange={e => { setBulkCatFilter(e.target.value); setSelectedForBulk([]) }}
                        style={{
                          width: '100%', padding: '8px 10px', fontSize: 13,
                          border: '1px solid #D5DFE9', borderRadius: 8, outline: 'none', background: '#fff',
                        }}
                  ...
                  <div style={{ fontSize: 12, color: '#8A9AB5', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontWeight: 700, color: '#1A3FA6' }}>{selectedForBulk.length || 'All'}</span>
                    {' '}{selectedForBulk.length === 1 ? 'product' : 'products'} selected
                    {' '}· Select individually below or leave unchecked to apply to all visible products
                  </div>

                  <div style={{ maxHeight: 280, overflow: 'auto', border: '1px solid #F0F4F8', borderRadius: 8 }}>
                    <DataTable
                      columns={bulkColumns}
                      rows={products.filter(p => !bulkCatFilter || p.category_id === parseInt(bulkCatFilter))}
                      rowKey={p => p.id}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  <div style={{ padding: '12px 16px', background: '#EAF0FB', borderRadius: 10, fontSize: 13, color: '#1A3FA6' }}>
                    ...
                    <code style={{ background: '#fff', padding: '1px 5px', borderRadius: 3 }}>sku</code>,{' '}
                    <code style={{ background: '#fff', padding: '1px 5px', borderRadius: 3 }}>selling_price</code>.
                  ...
                  <label style={{
                    ...
                    padding: '40px 20px', border: '2px dashed #D5DFE9', borderRadius: 12,
                    cursor: 'pointer', background: bulkFile ? '#F0F9F4' : '#F8FAFC',
                  }}>
                    <FileSpreadsheet size={36} color={bulkFile ? '#1A6E3A' : '#C8D4E0'} />
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: bulkFile ? '#1A6E3A' : '#3A4A5C' }}>
                        {bulkFile ? bulkFile.name : 'Drop CSV or Excel file here'}
                      </div>
                      <div style={{ fontSize: 12, color: '#8A9AB5', marginTop: 4 }}>or click to browse · .csv, .xlsx</div>
                    </div>
                    <input
                      type="file" accept=".csv,.xlsx,.xls" style={{ display: 'none' }} ref={bulkUpRef}
                      onChange={e => { if (e.target.files?.[0]) setBulkFile(e.target.files[0]) }}
                    />
                  </label>
                  {bulkFile && (
                    <Btn variant="ghost" onClick={() => setBulkFile(null)}>
      ...
      <Modal
        open={importModal}
        onClose={() => { setImportModal(false); setImportStep(1); setImportFile(null); setImportPreview(null) }}
        ...
        footer={
          importStep === 1 ? (
            <>
              ...
              <Btn
                onClick={() => { if (importFile) previewMut.mutate(importFile) }}
                disabled={!importFile || previewMut.isPending}
              >
            ...
            <>
              ...
              <Btn onClick={() => importMut.mutate(importFile)} disabled={importMut.isPending}>
                {importMut.isPending
                  ...
                  : `Confirm Import (${importPreview?.rows?.length ?? 0} rows)`
                }
              </Btn>
            </>
          )
      ...
      >
          ...
          {[{ n: 1, label: 'Upload File' }, { n: 2, label: 'Preview' }, { n: 3, label: 'Done' }].map(s => (
            <div key={s.n} style={{ flex: 1, textAlign: 'center' }}>
              <div style={{
                ...
                background: importStep >= s.n ? '#0A1628' : '#F0F4F8',
                color: importStep >= s.n ? '#E8A020' : '#8A9AB5',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: 12, fontWeight: 700, margin: '0 auto 4px',
              }}>{s.n}</div>
              <div style={{
                fontSize: 11,
                color: importStep >= s.n ? '#1C2B3A' : '#8A9AB5',
                fontWeight: importStep === s.n ? 700 : 400,
              }}>{s.label}</div>
            </div>
          ))}
        </div>
        ...
        {importStep === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <label style={{
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: importFile ? '#1A6E3A' : '#3A4A5C' }}>
            ...
            {importFile && (
              <div style={{
                ...
                padding: '8px 12px', background: '#EAF5EE', borderRadius: 8, fontSize: 12,
              }}>
        ...
        {importStep === 2 && importPreview && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{
              overflowX: 'auto', border: '1px solid #F0F4F8',
                  ...
                  <tr>
                    {(importPreview.headers ?? []).map((h, i) => (
                      <th key={i} style={{
                        ...
                        color: '#8A9AB5', textTransform: 'uppercase',
                        fontSize: 10, letterSpacing: '.5px', whiteSpace: 'nowrap',
                ...
                <tbody>
                  {(importPreview.rows ?? []).map((row, ri) => (
                    <tr key={ri} style={{ borderBottom: '0.5px solid #F8FAFC' }}>
                      {row.map((cell, ci) => (
                        <td key={ci} style={{ padding: '7px 12px', color: '#3A4A5C', whiteSpace: 'nowrap' }}>
                          {cell ?? '—'}
                        </td>
                      ))}
                    </tr>
            ...
            <div style={{ fontSize: 12, color: '#8A9AB5' }}>
              Showing first {importPreview.rows?.length ?? 0} rows · Duplicate SKUs will be skipped
            </div>
          </div>
        )}
      </Modal>
      ...
      {imageTarget && (
        <Modal
          ...
          title={`Upload Image — ${imageTarget?.name ?? ''}`}
          width={380}
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20, alignItems: 'center', padding: '8px 0' }}>
            ...
            <label style={{
              display: 'flex', alignItems: 'center', gap: 8,
              padding: '10px 24px', background: '#0A1628', color: '#E8A020',
      ...
      <Modal
        ...
        footer={
          <>
            <Btn variant="ghost" onClick={() => setOpeningStockModal(false)}>Cancel</Btn>
            <Btn
              onClick={() => { if (osFile && osBranch) openingStockMut.mutate({ file: osFile, branchId: osBranch }) }}
              disabled={!osFile || !osBranch || openingStockMut.isPending}
            >
              {openingStockMut.isPending ? <><Spinner size={12} /> Applying…</> : 'Apply Opening Stock'}
            </Btn>
          </>
        }
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ padding: '10px 14px', background: '#EAF0FB', borderRadius: 10, fontSize: 12, color: '#1A3FA6', lineHeight: 1.6 }}>
            <strong>Use this to set initial stock quantities</strong> when migrating from another system.<br />
            CSV columns: <code style={{ background: '#fff', padding: '1px 4px', borderRadius: 3 }}>sku</code>,{' '}
            <code style={{ background: '#fff', padding: '1px 4px', borderRadius: 3 }}>quantity</code>,{' '}
            <code style={{ background: '#fff', padding: '1px 4px', borderRadius: 3 }}>cost_price</code> (optional),{' '}
            <code style={{ background: '#fff', padding: '1px 4px', borderRadius: 3 }}>notes</code> (optional)
          </div>

          <FormField label="Select Branch *" required>
            <select
              value={osBranch}
              onChange={e => setOsBranch(e.target.value)}
              style={{ width: '100%', padding: '9px 12px', fontSize: 13, border: '1px solid #D5DFE9', borderRadius: 8, outline: 'none', background: '#fff', color: '#3A4A5C' }}
            >
              <option value="">— Choose branch —</option>
              {branches.map(b => <option key={b.id} value={b.id}>{b.is_head_office ? '🏢 ' : '📍 '}{b.name}</option>)}
            </select>
          </FormField>

          <Btn variant="ghost" onClick={downloadOpeningStockTemplate} style={{ alignSelf: 'flex-start' }}>
            <Download size={13} /> Download CSV Template
          </Btn>

          <label style={{
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10,
            padding: '28px 20px', border: `2px dashed ${osFile ? '#1A6E3A' : '#D5DFE9'}`,
            borderRadius: 12, cursor: 'pointer', background: osFile ? '#EAF5EE' : '#F8FAFC',
          }}>
            <Package size={28} color={osFile ? '#1A6E3A' : '#C8D4E0'} />
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: osFile ? '#1A6E3A' : '#3A4A5C' }}>
                {osFile ? osFile.name : 'Drop CSV file here or click to browse'}
              </div>
              <div style={{ fontSize: 11, color: '#8A9AB5', marginTop: 3 }}>.csv · max 5 MB</div>
            </div>
            <input type="file" accept=".csv,.txt" style={{ display: 'none' }}
              onChange={e => { if (e.target.files?.[0]) setOsFile(e.target.files[0]) }} />
          </label>

          {osFile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: '#EAF5EE', borderRadius: 8, fontSize: 12 }}>
              <span style={{ fontWeight: 600, color: '#1A6E3A' }}>✓ {osFile.name}</span>
              <span style={{ color: '#8A9AB5' }}>({(osFile.size / 1024).toFixed(1)} KB)</span>
              <button type="button" onClick={() => setOsFile(null)}
                style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: '#C0392B' }}>
                <X size={12} />
              </button>
            </div>
          ...
          <div style={{ padding: '10px 14px', background: '#FEF0E6', borderRadius: 8, fontSize: 12, color: '#855000' }}>
            ⚠️ This will <strong>overwrite</strong> existing stock quantities for this branch. Only run this during initial setup.
          </div>
        </div>
      </Modal>
      ...
      <Modal
        ...
        width={520}
        footer={<Btn variant="ghost" onClick={() => setPrintLabelModal(false)}>Close</Btn>}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ padding: '10px 14px', background: '#EAF0FB', borderRadius: 10, fontSize: 13, color: '#1A3FA6' }}>
            Select products below and generate printable barcode label sheets (A4, 3×8 grid).
          </div>
          <div style={{ padding: '40px 0', textAlign: 'center', color: '#8A9AB5', fontSize: 13 }}>
            <Tag size={32} color="#D5DFE9" style={{ marginBottom: 12, display: 'block', margin: '0 auto 12px' }} />
            <div style={{ fontWeight: 600, color: '#3A4A5C', marginBottom: 4 }}>Label Printing — Sprint 4</div>
            <div>Full barcode/QR label printing with custom templates is coming in the next sprint.</div>
          </div>
        </div>
      </Modal>
