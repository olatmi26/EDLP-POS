...
if [ "$HAS_LARAVEL" = true ]; then
  ...
  # Auto-install composer dependencies if composer.json changed
  if [ "$DRY_RUN" = false ] && [ -f "$PROJECT_ROOT/composer.json" ]; then
    if command -v composer &>/dev/null; then
      log_info "Running composer install --no-interaction..."
      (cd "$PROJECT_ROOT" && composer install --no-interaction --quiet 2>/dev/null) && \
        log_ok "Composer dependencies installed" || \
        log_warn "composer install had warnings — check manually"
    else
      log_warn "composer not in PATH — run: composer install"
    fi
  fi
